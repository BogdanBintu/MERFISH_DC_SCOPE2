...
class analysis_smFISH():
    ...
    def set_set(self,set_=''):
        self.set_ = set_
        self.fls_bk_ = [fl for fl in self.fls_bk if set_ in os.path.basename(os.path.dirname(fl))]
    def set_fov(self,ifl,set_=None):
        if set_ is not None:
            self.set_set(set_)
        self.fl_bk = self.fls_bk_[ifl]
    def set_hybe(self,iQ):
        self.Qfolder = [qfld for qfld in self.Qfolders if self.set_ in os.path.basename(qfld)][iQ]
    ...
    def get_Xh_simple(self,th = 4,s=30,delta_fit=4,dic_psf=None,normalized=False):
        resc=  5
        self.Xhs = []
        for im_raw in self.im_sig_[:-1]:
            im_ = norm_slice(im_raw,s=s)
            th_ = np.std(im_[::resc,::resc,::resc])*th
            self.Xhs.append(get_local_max(im_,th_,im_raw=im_raw,delta_fit=delta_fit,dic_psf=dic_psf))
    def get_Xh(self,th = 5,s=30,delta_fit=4,dic_psf=None,subtract_bk=False,trim0=True,fr=1.25):
        """
        ...
        for icol in range(self.ncols-1):
            ...
            if subtract_bk:
                ...
                if fr is None:
                    ...
                    imbk_resc = imbk[::resc,::resc,::resc]
                    imsg_resc = imsg[::resc,::resc,::resc]
                    #fr_ = np.min(imsg_resc/imbk_resc)
                    fr_ = np.percentile(imsg_resc/imbk_resc,5)
                    fr_ = np.min([1.25,fr_])
                    #fr_ = np.min([1.25,np.median(imsg[::resc,::resc,::resc]/imbk[::resc,::resc,::resc])])
                else:
                    fr_ = fr
                print("Fraction of bk:",fr_)
                    
                im_raw = imsg-imbk*fr_

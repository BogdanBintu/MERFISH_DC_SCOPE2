...
class Mosaic(object):
    ...
    def get_ims(self):
        self.ims = [dic["data"] if "data" in dic else dic["numpy_data"] for dic in self.dics]
    ...
    def save(self,filename,im_min=None,im_max=None):
        im=np.array(self.im,dtype=np.float16)
        if im_min==None:
            im_min=np.min(im)
        if im_max==None:
            im_max=np.max(im)
        ...
        im[im<0]=0
        im[im>1]=1
        if filename.split('.')[-1][:3]=='tif':
            import tifffile
            tifffile.imsave(filename,(im * 255).astype(np.uint8))
        else:
